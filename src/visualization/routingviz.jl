    ...
    if v.pathMode && !v.pathFrozen
        ...
        if nodeId != v.destNode
            if v.destNode != v.networkviz.selectedNode
                ...
                # red for new node
                sfCircleShape_setFillColor(v.nodes[nodeId], SFML.sfRed)
            end
            v.destNode = nodeId
...
function visualRedraw(v::RoutingViz)
    ...
    if v.pathMode
        # redraw the path
        drawPath(v)
        sfCircleShape_setFillColor(v.nodes[v.destNode], SFML.sfRed)
    end
end
...
function drawPath(v::RoutingViz)
    ...
    for line in v.path
        ...
        set_fillcolor(line, SFML.sfColor_fromRGB(0, 0, 125))
    end
end
